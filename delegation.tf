...
## Provision the service linked role when compute optimizer is enabled
resource "aws_iam_service_linked_role" "compute_optimizer" {
  count = var.enable_delegation.compute_optimizer != null ? 1 : 0

  aws_service_name = "compute-optimizer.amazonaws.com"
  description      = "Service-Linked Role for Compute Optimizer, enables organization-wide rightsizing recommendations"
  tags             = var.tags
}
